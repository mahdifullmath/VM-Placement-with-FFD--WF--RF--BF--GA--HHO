...
int runGA(idata PMserv, unsigned int iterationForAvg, unsigned int algIndex)
{
	...
	ga ga1(GA_epoch, GA_pop_size, GA_num_dims);
	ga1.initialize(PMserv.iVMs);
	if (VERBOSE)
		ga1.printPop();
	std::cout << "initialization of GA pop is finished!" << std::endl;
	if (VERBOSE)
		PMserv.printPMsWithPower();
	ga1.evolve(PMserv.iPMs, PMserv.iVMs, out_file_alg, iterationForAvg);
	std::cout << endl
			  << "===================END GA======================" << endl;
	...
	out_file_alg << ga1.Solution.power << "    " << ga1.Solution.wastage << "    " << ga1.Solution.numActivePMs << "    " << ga1.Solution.utilCPU << "    " << ga1.Solution.utilMEM << "    " << tExecution << endl;
	/*resultsAllAlgorithmsAllIterations[algIndex][0][iterationForAvg]=ga1.Solution.power;
	resultsAllAlgorithmsAllIterations[algIndex][1][iterationForAvg]=ga1.Solution.wastage;
	resultsAllAlgorithmsAllIterations[algIndex][2][iterationForAvg]=ga1.Solution.numActivePMs;
	resultsAllAlgorithmsAllIterations[algIndex][3][iterationForAvg]=ga1.Solution.utilCPU;
	resultsAllAlgorithmsAllIterations[algIndex][4][iterationForAvg]=ga1.Solution.utilMEM;
	resultsAllAlgorithmsAllIterations[algIndex][5][iterationForAvg]=(float) tExecution;
	*/
	saveResultsofIteration(iterationForAvg, algIndex, ga1.Solution.power, ga1.Solution.wastage, ga1.Solution.numActivePMs, ga1.Solution.utilCPU, ga1.Solution.utilMEM, tExecution);
